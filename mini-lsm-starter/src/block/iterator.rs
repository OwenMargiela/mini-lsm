...
impl BlockIterator {
    ...
    fn seek_to_offset(&mut self, offset: usize) {
        ...
        let prefix = entry.get_u16();
        let key_len = entry.get_u16() as usize;
        let key_slice = &entry[..key_len];

        let first_key = &self.first_key.raw_ref()[..prefix as usize];

        let mut concat_key = first_key.to_vec();
        concat_key.put(key_slice);

        self.key = KeyVec::from_vec(concat_key);
        ...
        let value_offset_begin = offset + U16_SIZE + U16_SIZE + key_len + U16_SIZE;
